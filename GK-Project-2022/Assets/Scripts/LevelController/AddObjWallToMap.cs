...
public class AddObjWallToMap
{
    ...
    public static void Generate2(ref List<List<int>> map, int _objectId)
    {
        ...
        for (int y = 0; y < map.Count; y++) 
            if (map[y][0] == (int)objectId.Wall && map[y][0 + 1] == (int)objectId.Air) avilible.Add(new Tuple<int, int>(0, y));
        for (int y = 0; y < map.Count; y++) 
            if (map[y][maxIndexY] == (int)objectId.Wall && map[y][maxIndexY - 1] == (int)objectId.Air) avilible.Add(new Tuple<int, int>(maxIndexY, y));
        for (int x = 0; x < map[0].Count; x++) 
            if (map[0][x] == (int)objectId.Wall && map[0 + 1][x] == (int)objectId.Air) avilible.Add(new Tuple<int, int>(x, 0));
        for (int x = 0; x < map[0].Count; x++) 
            if (map[maxIndexX][x] == (int)objectId.Wall && map[maxIndexX - 1][x] == (int)objectId.Air) avilible.Add(new Tuple<int, int>(x, maxIndexX));
        for (int y = 1; y < map.Count - 1 - 1; y++)
        {
            for (int x = 1; x < map[y].Count - 1 - 1; x++)
            {
                if (map[y][x] != (int)objectId.Wall) continue;
                if (map[y][x + 1] == (int)objectId.Air || map[y][x - 1] == (int)objectId.Air || map[y + 1][x] == (int)objectId.Air || map[y - 1][x] == (int)objectId.Air)
                    avilible.Add(new Tuple<int, int>(x, y));
            }
        }
        //Choose random avilible cord
